...
export const getNormalizedBlocks = (
  partialContextOrComponents:
    | Pick<RenderContext, 'extensions' | 'page'>
    | ReturnType<typeof getComponents>
) => {
  if (
    'extensions' in partialContextOrComponents &&
    'page' in partialContextOrComponents
  ) {
    const partialContext = partialContextOrComponents as Pick<
      RenderContext,
      'extensions' | 'page'
    >
    return normalize(
      getComponents(
        partialContext.extensions,
  ...
  const components = partialContextOrComponents as ReturnType<
